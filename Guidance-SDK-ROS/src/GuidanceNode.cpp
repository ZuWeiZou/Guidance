...
#include <geometry_msgs/Twist.h> //vector
#include <math.h>//


FILE *pAngle=fopen("/home/ubuntu/catkin_ws/src/Guidance-SDK-ROS/data/position_data.txt","w");
FILE *pPosition = fopen("/home/ubuntu/catkin_ws/src/Guidance-SDK-ROS/data/position_data.txt","w");
...
#define RED_START_POINT_X 0
#define RED_START_POINT_Y 0
#define RED_START_POINT_Z 0
...
bool            show_images = 0;
uint8_t         verbosity = 0;
e_vbus_index	CAMERA_ID = e_vbus5;//bottom camera
DJI_lock        g_lock;
DJI_event       g_event;
...
//            printf( "frame index: %d, stamp: %d\n", oa->frame_index, oa->time_stamp );
 //           printf( "obstacle distance:" );
            ...
            {
                printf( " %f cm", 0.01f * oa->distance[i] );
            }
            printf( "\n" );
        }
...
/*    if ( e_ultrasonic == data_type && NULL != content )
    ...
    if(e_motion == data_type && NULL!=content){
            float quat[4]={0},angle[3]={0};
            //quat[0], quat[1],quat[2],quat[3];
            float c32, c33,c31,c11, c21;
            ...
            angle[2]=57.3*atan2(c21,c11);  //-PI~PI
            angle[1]=57.3*asin(-c31);	 //-PI/2~PI/2
            angle[0]=57.3*atan2(c32,c33);	//-PI~PI

	        if(angle[2]<0) angle[2]+=2*3.14;
		    
		    //printf("frame index: %d, stamp: %d\n", m->frame_index, m->time_stamp);
//		    printf("(m->position_in_global_x,m->position_in_global_y,m->position_in_global_z)=(%.2f,%.2f,%.2f)\n", m->position_in_global_x, m->position_in_global_y, m->position_in_global_z);
//           printf("(angle[0],angle[1],angle[2])=(%.2f,%.2f,%.2f)\n", angle[0], angle[1], angle[2]);
//		    // publish position
		    geometry_msgs::Twist g_pos;
            geometry_msgs::TransformStamped g_vimu;
		    //g_pos.header.frame_id = "guidance";
		    //g_pos.header.stamp = ros::Time::now();

            g_pos.linear.x = m->position_in_global_x;
		    g_pos.linear.y = m->position_in_global_y;
		    g_pos.linear.z = m->position_in_global_z;

		    g_pos.angular.x = angle[0];
		    g_pos.angular.y = angle[1];
		    g_pos.angular.z = angle[2];
		    position_pub.publish(g_pos);

		    g_vimu.transform.rotation.w = quat[0];
		    g_vimu.transform.rotation.x = quat[1];
		    g_vimu.transform.rotation.y = quat[2];
		    g_vimu.transform.rotation.z = quat[3];
            vimu_pub.publish(g_vimu);

		    fprintf(pAngle,"oula angle %lf x %lf y %lf z %f\n",g_pos.linear.x,g_pos.linear.y,g_pos.linear.z);
            fprintf(pPosition,"Position %lf x %lf y %lf z %f\n",g_pos.angular.x,g_pos.angular.y,g_pos.angular.z);
    }
...
//    velocity_pub  			= my_node.advertise<geometry_msgs::Vector3Stamped>("/guidance/velocity",1);
    ...
    position_pub            = my_node.advertise<geometry_msgs::Twist>("/guidance/position", 1);
    vimu_pub                = my_node.advertise<geometry_msgs::TransformStamped>("/guidance/vimu", 1);
    /* initialize guidance */
    reset_config();
	...
	sleep(400);  //1 ,test how long can i relesae?
